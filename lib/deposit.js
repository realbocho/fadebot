...
// Gasless: wrap any raw USDC sitting in the deposit wallet into pUSD via the
// CollateralOnramp, executed as a relayer WALLET batch from the deposit wallet.
export async function wrapToTradable(wallet, depositWallet, onProgress) {
  const provider = getProvider();
  const relayer = buildRelayer(wallet);
  const calls = [];

  for (const asset of [USDC_E, USDC_NATIVE]) {
    const c = new Contract(asset, ERC20_IFACE, provider);
    const raw = await c.balanceOf(depositWallet);
    ...
    calls.push({
      target: ONRAMP,
      value: "0",
      data: ONRAMP_IFACE.encodeFunctionData("wrap", [asset, depositWallet, raw]),
    });
  }
  if (!calls.length) return { wrapped: 0 };

  onProgress?.("Converting to trading balance (gasless)…");
  const deadline = String(Math.floor(Date.now() / 1000) + 600);
  const res = await relayer.executeDepositWalletBatch(calls, depositWallet, deadline);
  await res.wait();
  return { wrapped: calls.length / 2 };
}
