...
export async function withdrawFromDepositWallet(wallet, depositWallet, to, amountUsd, onProgress) {
  ...
  const calls = [
    ...
    },
    {
      target: OFFRAMP,
      value: "0",
      data: OFFRAMP_IFACE.encodeFunctionData("unwrap", [USDC_E, to, amount]),
    },
  ];
  await runBatchConfirmed(relayer, calls, depositWallet, "Withdraw", onProgress);
  return { withdrawn: Number(utils.formatUnits(amount, 6)), token: "USDC.e", to };
}
