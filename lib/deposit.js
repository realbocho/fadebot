...
// pUSD → USDC.e (CollateralOfframp.unwrap, address from docs.polymarket.com/resources/contracts)
...
const OFFRAMP_IFACE = new utils.Interface([
...
export async function getFundingBreakdown(depositWallet) {
  ...
  try {
    ...
    // An RPC failure must not read as "$0" — that sends users chasing a
    // deposit that may have arrived fine.
    throw new Error(
      "Couldn't check your on-chain balance (RPC error). This is usually the " +
      "public Polygon RPC rate-limiting — set NEXT_PUBLIC_POLYGON_RPC to a " +
      "private endpoint (free Alchemy tier works) and redeploy. " +
      (e?.message ? `Details: ${e.message}` : "")
    );
  }
}

// Execute a deposit-wallet batch and wait for FULL confirmation. wait() can
// resolve at STATE_MINED while the relayer still holds an "active action" on
// the wallet — submitting the next batch then 400s with "wallet busy". Poll
// to STATE_CONFIRMED, and retry submission a few times if a previous action
// is still draining.
async function runBatchConfirmed(relayer, calls, depositWallet, label, onProgress) {
  const deadline = () => String(Math.floor(Date.now() / 1000) + 600);
  let lastErr;
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      lastErr = e;
      if (/wallet busy|active action/i.test(JSON.stringify(e?.data ?? "") + msg)) {
        onProgress?.(`${label} — wallet busy, retrying in 15s… (${attempt + 1}/5)`);
...
export async function wrapToTradable(wallet, depositWallet, onProgress) {
  ...
  if (!nativeBal.isZero()) {
    ...
    await runBatchConfirmed(relayer, swapCalls, depositWallet, "Swap", onProgress);
  }
  ...
  await runBatchConfirmed(relayer, calls, depositWallet, "Convert", onProgress);
  return { wrapped: 1 };
}

// Gasless withdrawal: unwrap pUSD in the deposit wallet back to USDC.e and
// send it straight to an external Polygon address — unwrap's `_to` is the
// final recipient, so approve + unwrap is the whole batch. `amountUsd` in
// dollars; pass null/undefined to withdraw the full pUSD balance.
export async function withdrawFromDepositWallet(wallet, depositWallet, to, amountUsd, onProgress) {
  if (!/^0x[a-fA-F0-9]{40}$/.test(to || ""))
    throw new Error("Invalid destination address — paste a Polygon (0x…) address.");
  if (to.toLowerCase() === depositWallet.toLowerCase())
    throw new Error("Destination is your trading address itself — paste an external wallet.");
  ...
  const pusd = new Contract(CONFIG.collateral, ERC20_IFACE, provider);
  const bal = await pusd.balanceOf(depositWallet);
  if (bal.isZero()) throw new Error("Nothing to withdraw — tradable balance is $0.");

  let amount = bal;
  if (amountUsd != null) {
    amount = utils.parseUnits(String(amountUsd), 6);
    if (amount.lte(0)) throw new Error("Withdrawal amount must be positive.");
    if (amount.gt(bal))
      throw new Error(
        `Amount exceeds balance — you have $${Number(utils.formatUnits(bal, 6)).toFixed(2)} withdrawable.`
      );
  }

  onProgress?.("Withdrawing (gasless)…");
  // Recipients expect native USDC (exchanges are dropping USDC.e), so:
  // pUSD → unwrap to USDC.e in the deposit wallet → swap to native USDC with
  // the destination as the swap recipient. 0.5% min-out guard on a deep 1:1
  // stable pool; the relayer simulates the whole batch, so a bad route fails
  // safely before any funds move. Fee tiers tried: 0.01% then 0.05%.
  const minOut = amount.mul(10000 - SWAP_SLIPPAGE_BPS).div(10000);
  const buildCalls = (feeTier) => [
    {
      target: CONFIG.collateral,
      value: "0",
      data: ERC20_IFACE.encodeFunctionData("approve", [OFFRAMP, amount]),
    },
    {
      target: OFFRAMP,
      value: "0",
      data: OFFRAMP_IFACE.encodeFunctionData("unwrap", [USDC_E, depositWallet, amount]),
    },
    {
      target: USDC_E,
      value: "0",
      data: ERC20_IFACE.encodeFunctionData("approve", [UNIV3_ROUTER, amount]),
    },
    {
      target: UNIV3_ROUTER,
      value: "0",
  ...
  try {
    await runBatchConfirmed(relayer, buildCalls(UNIV3_FEE), depositWallet, "Withdraw", onProgress);
  } catch (e) {
    if (!/revert/i.test(String(e?.message || e))) throw e;
    onProgress?.("Retrying via a different pool…");
    await runBatchConfirmed(relayer, buildCalls(500), depositWallet, "Withdraw", onProgress);
  }
  return { withdrawn: Number(utils.formatUnits(amount, 6)), token: "USDC", to };
}

// ── Claim winnings (redeem) ─────────────────────────────────────
// V2 redemption routes through the collateral adapters
// (docs.polymarket.com/trading/ctf/redeem): the adapter burns the outcome
// tokens via the CTF, wraps the released USDC.e, and returns pUSD to the
// deposit wallet. Standard and neg-risk markets use different adapters but
// the same 4-arg signature. Addresses from docs.polymarket.com/resources/contracts.
const CTF_COLLATERAL_ADAPTER = "0xada100db00ca00073811820692005400218fce1f";
const NEGRISK_COLLATERAL_ADAPTER = "0xada2005600dec949baf300f4c6120000bdb6eaab";
const ZERO32 = "0x" + "0".repeat(64);
const REDEEM_IFACE = new utils.Interface([
  "function redeemPositions(address collateralToken, bytes32 parentCollectionId, bytes32 conditionId, uint256[] indexSets)",
]);

// Redeem one or more resolved markets in a single gasless batch.
// `items`: [{ conditionId, negRisk }]. Grants the adapters' one-time ERC1155
// approval inside the same batch if missing.
export async function redeemWinnings(wallet, depositWallet, items, onProgress) {
  if (!items?.length) throw new Error("Nothing to claim.");
  const provider = getProvider();
  const relayer = buildRelayer(wallet);
  const ctf = new Contract(CONFIG.conditionalTokens, ERC1155_IFACE, provider);

  const adaptersNeeded = [...new Set(items.map((i) =>
    i.negRisk ? NEGRISK_COLLATERAL_ADAPTER : CTF_COLLATERAL_ADAPTER
  ))];

  const calls = [];
  for (const adapter of adaptersNeeded) {
    const ok = await ctf.isApprovedForAll(depositWallet, adapter);
    if (!ok)
      calls.push({
        target: CONFIG.conditionalTokens,
        value: "0",
        data: ERC1155_IFACE.encodeFunctionData("setApprovalForAll", [adapter, true]),
      });
  }

  for (const item of items) {
    const adapter = item.negRisk ? NEGRISK_COLLATERAL_ADAPTER : CTF_COLLATERAL_ADAPTER;
    calls.push({
      target: adapter,
      value: "0",
      data: REDEEM_IFACE.encodeFunctionData("redeemPositions", [
        CONFIG.collateral, ZERO32, item.conditionId, [1, 2],
      ]),
    });
  }

  onProgress?.(`Claiming winnings — ${items.length} market${items.length > 1 ? "s" : ""} (gasless)…`);
  await runBatchConfirmed(relayer, calls, depositWallet, "Claim", onProgress);
  return { redeemed: items.length };
}
