...
export function smartMoneySummary(positions, whaleByAddress, minUsd = cfg.xrayMinUsd) {
  ...
  for (const p of positions) {
    ...
    // Only surface as a fade alert if the wallet GENUINELY looks cold right now:
    // a real losing streak, or a low win rate over enough settled positions.
    // A stale "fade" tag alone (e.g. from a past harvest) isn't enough — showing
    // "1-loss streak · WR 50%" as a fade signal just erodes trust.
    const genuinelyCold =
      w.streak <= -cfg.fadeStreak ||
      (w.win_rate != null && w.win_rate <= cfg.maxFadeWinRate && w.closed_count >= cfg.minClosed);
    if (w.tier === "fade" && genuinelyCold) fadeAlerts.push(entry);
    else {
      tracked.push(entry);
