...
export async function fetchWalletStats(address) {
  const rows = firstList(
    await getJson(`${DATA_API}/v1/closed-positions`, {
      user: address,
      limit: 200,
      sortBy: "TIMESTAMP",
      sortDirection: "DESC",
  ...
  // closed-positions exposes realizedPnl per settled position. Sort by TIMESTAMP
  // (not the default REALIZEDPNL) so streaks are chronological, and read the
  // signed realizedPnl for win/loss. If this endpoint ever returns winners-only,
  // win_rate will read 1.0 — the diagnostic route checks for exactly that.
  const results = rows
    .map((p) => ({
