...
export async function fetchMarketPositions(conditionId) {
  ...
  return rows.map((p) => ({
    ...
    totalPnl: Number(pick(p, "totalPnl", "cashPnl") ?? 0),
  }));
}
...
// ── Fallback x-ray: no tracked whales in this market? Use the market's own
// profitable holders — market-positions is already sorted by TOTAL_PNL.
export function marketNativeSummary(positions, minUsd = cfg.xrayMinUsd, top = 8) {
  ...
  const byOutcome = {};
  for (const p of winners) byOutcome[p.outcome] = (byOutcome[p.outcome] || 0) + p.value;
