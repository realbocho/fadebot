...
export async function fetchWalletStats(address) {
  ...
  // Win/loss must come from the position's actual P&L. cashPnl is the reliable
  // signed total on closed-positions; realizedPnl has shown sign/scaling quirks
  // (it read as always-positive, making every wallet look 100% — the bug behind
  // the uniform "WR 100% · 50W" rows). Prefer cashPnl, fall back carefully.
  const results = rows
    .map((p) => {
      let pnl = Number(pick(p, "cashPnl", "totalPnl") ?? NaN);
      if (!Number.isFinite(pnl)) {
        // Derive from value change if no signed P&L field is present.
        const cur = Number(pick(p, "currentValue") ?? NaN);
        const init = Number(pick(p, "initialValue") ?? NaN);
        if (Number.isFinite(cur) && Number.isFinite(init)) pnl = cur - init;
      }
      return { pnl, ts: pick(p, "endDate", "timestamp") ?? 0 };
    })
...
export async function harvestLoserCandidates({
  ...
  for (const m of trending) {
    ...
    diag.push({ slug: m.slug, positions: positions.length, hits, worstLossSeen: Math.round(최악) });
